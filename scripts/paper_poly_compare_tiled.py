...
# Add src first because paper_poly_compare imports sumcheck_cuda_ext.
...
def build_tiled_call(tables, chals, poly_name):
    poly_id = base.POLY_IDS[poly_name]

    def fn():
        return sumcheck_cuda_ext.sumcheck_hyperplonk_tiled_u32_cuda(
...
def main():
    ...
    if not hasattr(sumcheck_cuda_ext, "sumcheck_hyperplonk_tiled_u32_cuda"):
        raise RuntimeError("extension does not expose sumcheck_hyperplonk_tiled_u32_cuda")
    ...
    widths = [24, 10, 4, 12, 10, 10, 10, 11, 10, 10, 12]
    ...
    for nv in num_vars_list:
        ...
        for poly in poly_names:
            if poly not in base.POLYS:
                raise ValueError(f"unknown poly {poly}")
            ...
            spec_fn = base.build_specialized_call(tables, chals, poly)
            tiled_fn = build_tiled_call(tables, chals, poly)

            _gf, gmed, _gp90, gout = bench(generic_fn, args.warmup, args.runs)
            _sf, smed, sp90, sout = bench(spec_fn, args.warmup, args.runs)
            _tf, tmed, tp90, tout = bench(tiled_fn, args.warmup, args.runs)
            ...
            if args.check:
                ...
                if not torch.equal(g, s):
                    ...
                    raise AssertionError(f"generic/spec mismatch {poly} nv={nv} first={diff}")

                if not torch.equal(g, t):
                    diff = (g != t).nonzero()[0].tolist()
                    raise AssertionError(f"generic/tiled mismatch {poly} nv={nv} first={diff}")
